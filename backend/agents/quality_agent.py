...
class QualityGuardAgent(BaseAgent):
    """
    Final gate before a pipeline run is marked successful.
    Computes a 0–100 quality score across 5 dimensions.
    Asks the LLM for a narrative assessment and approval decision.
    Saves an HTML quality report to logs/quality_{run_id}.html.
    """
    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        narrative, approved, llm_thought = self._llm_assess(state, scores, total, threshold)
    ...
    def _compute_scores(self, state: Dict) -> Tuple[Dict[str, float], Dict[str, Any]]:
        ...
        details["row_volume"] = {"final_rows": final_rows, "target": self.MIN_LISTINGS, "score": d1, "max": w}
        ...
        d2         = round(w * (n_success / n_total), 2)
        if has_synth:
            d2 = round(d2 * 0.5, 2)
        rungs       = [r.get("fallback_rung", "") for r in listings_results.values() if r.get("status") == "fallback"]
        ...
        details["data_freshness"] = {"sources_succeeded": n_success, "sources_fallback": n_fallback, "has_synthetic": has_synth, "fallback_rungs": rungs, "score": d2, "max": w}

        # D3: Completeness
        w = self.WEIGHTS["completeness"]
        critical_cols = {"price": 0.40, "surface_m2": 0.45, "governorate": 0.20, "property_type": 0.25}
        col_scores = []
        col_details = {}
        for col, thr in critical_cols.items():
            rate  = null_rate.get(col, 0.0)
            col_s = 1.0 if rate <= thr else max(0.0, 1.0 - (rate - thr) / (1.0 - thr))
        ...
        if susp_rate <= 0.02:   d4 = w
        elif susp_rate <= 0.10: d4 = round(w * (1 - (susp_rate - 0.02) / 0.08), 2)
        else:                   d4 = 0.0
        scores["consistency"]  = d4
        details["consistency"] = {"suspicious_rows": suspicious, "suspicious_rate": round(susp_rate, 4), "score": d4, "max": w}

        # D5: Source balance
        w          = self.WEIGHTS["source_balance"]
        row_counts = [r.get("rows", 0) for r in listings_results.values() if r.get("rows", 0) > 0]
        if len(row_counts) >= 2:
        ...
        details["source_balance"] = {"source_row_counts": row_counts, "score": d5, "max": w}

        return scores, details

    def _llm_assess(self, state: Dict, scores: Dict[str, float], total: float, threshold: int) -> Tuple[str, bool, str]:
        cleaning_report  = state.get("cleaning_report", {})
        listings_results = state.get("listings_results", {})

        source_summary = {
            name: {"status": r.get("status"), "rows": r.get("rows", 0), "fallback_rung": r.get("fallback_rung", "")}
        ...
        prompt = (
            ...
            f"Dimension scores:\n"
            + "\n".join(f"  {dim:<18}: {s:.1f}/{self.WEIGHTS[dim]}" for dim, s in scores.items())
            + f"\n\nData context:\n"
            ...
            f"Write a 3-sentence quality assessment.\n"
    ...
    def _save_html_report(self, report: Dict) -> str:
        ...
        def dim_row(dim, score):
            ...
            detail    = self._detail_str(dim, details.get(dim, {}))
            return (
                ...
                f"<td style='padding:8px 12px;font-size:12px;color:#666'>{detail}</td>"
                f"</tr>"
            )

        rows_html  = "\n".join(dim_row(d, s) for d, s in scores.items())
        narrative  = report.get("narrative", "").replace("\n", "<br>")
        null_info  = details.get("completeness", {}).get("columns", {})
        null_rows  = "\n".join(
...
<style>
  ...
  .score-big {{ font-size: 52px; font-weight: 700; color: {score_color}; line-height: 1; display: inline-block; }}
  .score-label {{ font-size: 14px; color: #666; vertical-align: bottom; padding-bottom: 8px; margin-left: 6px; }}
  .card {{ background: #f8f8f8; border-radius: 10px; padding: 18px 22px; margin-bottom: 18px; }}
  table {{ border-collapse: collapse; width: 100%; }}
  tr:nth-child(even) {{ background: #fafafa; }}
  th {{ text-align: left; padding: 8px 12px; font-size: 12px; font-weight: 500; color: #888; border-bottom: 1px solid #e8e8e8; }}
